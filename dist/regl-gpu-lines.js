(function (global, factory) {
  ...
  const ORIENTATION$3 = require$$6;
  var glslPrelude$2 = `
#ifndef PI
#define PI ${Math.PI}
#endif

#define CAP_START ${ORIENTATION$3.CAP_START}.0
#define CAP_END ${ORIENTATION$3.CAP_END}.0
...
bool isSelfIntersection(vec2 tBC, vec2 tCD, float widthC, float lBCD) {
  ...
  const glslPrelude$1 = glslPrelude$2;
...
void main() {
  ...
  const ORIENTATION$2 = require$$6;
  const glslPrelude = glslPrelude$2;
    ...
    return function drawLines(props) {
      ...
      function flush(props) {
        userConfig(props, () => {
          ...
          if (allRoundedSegments.length) drawRoundedSegment(allRoundedSegments); //if (allMiterSegments.length) drawMiterSegment(allMiterSegments);

          if (allRoundedCaps.length) drawRoundedCap(allRoundedCaps); //if (allMiterCaps.length) drawMiterCap(allMiterCaps);
